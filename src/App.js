...
function useApi() {
  const { user } = useAuth();
  return useCallback(async (method, url, data, extraConfig) => {
    const headers = user?.token
      ? { Authorization: `Bearer ${user.token}` }
      : {};
    const config = { method, url: `${API}${url}`, headers, ...extraConfig };
...
function KpiCard({ label, value, unit, color, sub }) {
  return (
    <div style={{
      ...
      <div style={{ color: C.muted, fontSize: 10, letterSpacing: 2, textTransform: "uppercase", marginBottom: 5 }}>
        {label}
      </div>
      <div style={{ color, fontSize: "1.65rem", fontWeight: 700, fontFamily: "monospace" }}>
        {value ?? "—"}
        {unit && <span style={{ fontSize: "0.85rem", marginLeft: 3 }}>{unit}</span>}
      </div>
...
function Btn({ children, onClick, color, disabled, size, outline }) {
  const borderColor = color || C.supply;
  return (
    <button
      onClick={onClick}
      disabled={disabled}
      style={{
        background: outline ? "transparent" : `${borderColor}22`,
  ...
  return (
    <nav style={{
      ...
      >
        ...
        <span style={{ fontWeight: 800, fontSize: 13, color: "white", letterSpacing: 0.5, whiteSpace: "nowrap" }}>
          Water Management Analytics
        </span>
      </div>

      {/* Nav links */}
      <div style={{ display: "flex", gap: 2, alignItems: "center", overflow: "auto", flexShrink: 1 }}>
        {navItems.map(n => (
          <button
            key={n.id}
      ...
      <div style={{ display: "flex", alignItems: "center", gap: 8, flexShrink: 0 }}>
        ...
        <span style={{ color: C.muted, fontSize: 12, whiteSpace: "nowrap" }}>{user?.name}</span>
...
function LoginPage({ onSwitchToSignup }) {
  ...
  const handleLogin = async (e) => {
    ...
    try {
      ...
      setErr(ex.response?.data?.error || "Login failed. Please check your credentials.");
    } finally {
      setLoading(false);
    }
  };
    ...
    <div style={{
      ...
      <div style={{
        ...
        <div style={{ textAlign: "center", marginBottom: 28 }}>
          ...
          <h1 style={{ fontSize: "1.5rem", fontWeight: 800, color: "white", margin: 0, marginBottom: 5 }}>
            Water Management Analytics
          </h1>
          <p style={{ color: C.muted, fontSize: 13, margin: 0 }}>
            Panvel Municipal Corporation · Sign in to continue
          </p>
        </div>
        ...
        <form onSubmit={handleLogin}>
          <div style={{ marginBottom: 14 }}>
            <label style={{ color: C.muted, fontSize: 11, letterSpacing: 1.5, display: "block", marginBottom: 5 }}>
              EMAIL ADDRESS
            </label>
            <input
              type="email"
    ...
    <div style={{
      ...
      <div style={{
        ...
        <div style={{ textAlign: "center", marginBottom: 24 }}>
          ...
          <h1 style={{ fontSize: "1.4rem", fontWeight: 800, color: "white", margin: 0, marginBottom: 5 }}>
            Create Account
          </h1>
        ...
        <form onSubmit={handleSignup}>
          ...
          <div style={{ marginBottom: 22 }}>
            <label style={{ color: C.muted, fontSize: 11, letterSpacing: 1.5, display: "block", marginBottom: 10 }}>
              SELECT YOUR ROLE
            </label>
            <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 10 }}>
              {[
                { id: "student", icon: "🎓", title: "Student",  desc: "View analytics & download data" },
                { id: "admin",   icon: "⚙",  title: "Admin",    desc: "Upload files & manage all data" },
              ].map(r => (
                <div
                  key={r.id}
...
function CityPage() {
  ...
  const predict = async () => {
    ...
    setLoading(true);
    try {
      const result = await api("get", `/predict/city/${days}`);
      setData(result);
    } catch (e) {
  ...
  const avg = (key) =>
    data.length
      ? (data.reduce((a, r) => a + (r[key] || 0), 0) / data.length).toFixed(2)
      : null;

  return (
    <div>
      <h2 style={{ color: "white", marginBottom: 4 }}>🏙 City Forecast</h2>
      <p style={{ color: C.muted, marginBottom: 20, fontSize: 13 }}>
        Predict total daily water supply for entire Panvel city.
      </p>
      <ErrBanner msg={err} />

      <div style={{ display: "flex", gap: 12, alignItems: "flex-end", marginBottom: 22, flexWrap: "wrap" }}>
        <div>
          <label style={{ color: C.muted, fontSize: 11, letterSpacing: 1.5, display: "block", marginBottom: 5 }}>
            FORECAST DAYS
          </label>
          <select
            ...
            style={{
              background: C.surface2, color: "white",
        ...
        <Btn onClick={predict} disabled={loading}>
      ...
      {loading ? (
        ...
        <>
          <div style={{ display: "flex", gap: 12, flexWrap: "wrap", marginBottom: 22 }}>
            <KpiCard label="Avg Supply"      value={avg("Predicted_Supply_MLD")}      unit="MLD" color={C.supply}      />
            <KpiCard label="Avg Consumption" value={avg("Predicted_Consumption_MLD")} unit="MLD" color={C.consumption} />
            <KpiCard label="Avg Leakage"     value={avg("Predicted_Leakage_MLD")}     unit="MLD" color={C.leakage}     />
            <KpiCard label="Avg Leakage %"   value={(data.reduce((a, r) => a + r.Leakage_Percentage, 0) / data.length).toFixed(1)} unit="%" color={C.warning} />
          </div>

          <div style={{ background: C.surface, borderRadius: 10, padding: 18, marginBottom: 18, border: "1px solid rgba(0,212,255,0.08)" }}>
            <h4 style={{ color: "#ccd6f6", margin: "0 0 12px 0" }}>Supply & Consumption</h4>
                ...
                {data.map((r, i) => (
                  <tr key={i} style={{ borderBottom: "1px solid rgba(255,255,255,0.03)", background: i % 2 ? C.surface2 : "transparent" }}>
                    ...
                    <td style={{ padding: "9px 12px" }}>
                      <span style={{
                        background: r.Leakage_Percentage >= 15 ? "rgba(255,77,109,0.15)" : "rgba(0,255,136,0.1)",
                        color: r.Leakage_Percentage >= 15 ? C.leakage : C.consumption,
...
function WardPage() {
  ...
  const predict = async () => {
    ...
    setLoading(true);
    try {
      setData(await api("get", `/predict/ward/${selected}/${days}`));
    } catch (e) {
      setErr(e.response?.data?.error || e.message);
    } finally {
      setLoading(false);
  ...
  const leakPct    = data[0]?.Leakage_Percentage;
  const levelColor = leakPct >= 20 ? "#ff4d6d" : leakPct >= 15 ? "#ffb800" : leakPct >= 10 ? "#ffd700" : "#00ff88";
  const levelLabel = leakPct >= 20 ? "CRITICAL" : leakPct >= 15 ? "HIGH" : leakPct >= 10 ? "MODERATE" : "NORMAL";

  return (
    <div>
      <h2 style={{ color: "white", marginBottom: 4 }}>🏘 Zone Forecast</h2>
      <p style={{ color: C.muted, marginBottom: 20, fontSize: 13 }}>
        Predict water supply for any individual zone.
      </p>
      <ErrBanner msg={err} />
      ...
      <div style={{ display: "flex", gap: 12, alignItems: "flex-end", marginBottom: 22, flexWrap: "wrap" }}>
        <div>
          ...
          <select
            ...
            style={{ background: C.surface2, color: "white", border: "1px solid rgba(0,212,255,0.2)", borderRadius: 6, padding: "9px 12px", fontSize: 13, minWidth: 220, fontFamily: "inherit" }}
          >
            {wardList.map(w => (
              <option key={w.Ward_No} value={w.Ward_No}>Zone {w.Ward_No} — {w.Ward_Name}</option>
            ))}
          </select>
        </div>
        <div>
          ...
          <select
            value={days}
            onChange={e => setDays(Number(e.target.value))}
            style={{ background: C.surface2, color: "white", border: "1px solid rgba(0,212,255,0.2)", borderRadius: 6, padding: "9px 12px", fontSize: 13, fontFamily: "inherit" }}
          >
      ...
      {loading ? (
        ...
        <>
          <div style={{ display: "flex", gap: 12, flexWrap: "wrap", marginBottom: 22, alignItems: "stretch" }}>
            ...
            <div style={{ background: C.surface, border: `2px solid ${levelColor}`, borderRadius: 8, padding: "14px 20px", display: "flex", flexDirection: "column", alignItems: "center", justifyContent: "center", minWidth: 120 }}>
              <span style={{ color: C.muted, fontSize: 10, letterSpacing: 2 }}>ALERT LEVEL</span>
              <span style={{ color: levelColor, fontWeight: 800, fontSize: "1.3rem", marginTop: 4 }}>{levelLabel}</span>
            </div>
          </div>
...
function AlertsPage() {
  ...
  const scan = async () => {
    ...
    setLoading(true);
    try {
      setData(await api("get", `/alerts/scan?days=${days}`));
    } catch (e) {
      setErr(e.response?.data?.error || e.message);
    } finally {
      setLoading(false);
  ...
  const levelStyle = (lvl) => {
    const m = {
      CRITICAL: ["#ff4d6d", "rgba(255,77,109,0.12)"],
      HIGH:     ["#ffb800", "rgba(255,184,0,0.1)"],
      MODERATE: ["#ffd700", "rgba(255,215,0,0.08)"],
      NORMAL:   ["#00ff88", "rgba(0,255,136,0.08)"],
    };
  ...
  return (
    <div>
      <h2 style={{ color: "white", marginBottom: 4 }}>🔔 Leakage Alert Scan</h2>
      <p style={{ color: C.muted, marginBottom: 20, fontSize: 13 }}>
        Scan all 27 zones and classify leakage alert levels.
      </p>
      <ErrBanner msg={err} />
      ...
      <div style={{ display: "flex", gap: 12, alignItems: "flex-end", marginBottom: 22 }}>
        ...
        <Btn onClick={scan} disabled={loading} color={C.warning}>
          {loading ? "⏳ Scanning..." : "🔍 SCAN ALL ZONES"}
        </Btn>
      </div>
            ...
            return (
              ...
              >
                <div>
                  ...
                  <span style={{ color: "white", marginLeft: 10, fontWeight: 600 }}>
                    Zone {r.Ward_No} — {r.Ward_Name}
                  </span>
                </div>
                <div style={{ display: "flex", gap: 16, fontSize: 12, color: C.muted }}>
                  <span>Avg: <b style={{ color: col }}>{r.Avg_Leakage_Pct}%</b></span>
                  <span>Max: <b style={{ color: col }}>{r.Max_Leakage_Pct}%</b></span>
                  <span>Days exceeded: <b style={{ color: "white" }}>{r.Days_Exceeding}</b></span>
...
function HistoricalPage() {
  ...
  return (
    <div>
      <h2 style={{ color: "white", marginBottom: 4 }}>📅 Historical Data</h2>
      <p style={{ color: C.muted, marginBottom: 20, fontSize: 13 }}>
        181 days of city-level water supply — September 2025 to February 2026.
      </p>
      <ErrBanner msg={err} />
      {loading ? (
        <Spinner />
...
function MetricsPage() {
  ...
  return (
    <div>
      <h2 style={{ color: "white", marginBottom: 4 }}>📊 Model Metrics</h2>
      <p style={{ color: C.muted, marginBottom: 20, fontSize: 13 }}>
        R² accuracy scores for all 29 Gradient Boosting models.
      </p>
      {loading ? (
        <Spinner />
      ) : (
        <>
          ...
          <div style={{ background: C.surface, borderRadius: 10, padding: 18, marginBottom: 18, border: "1px solid rgba(0,212,255,0.08)" }}>
            <ResponsiveContainer width="100%" height={240}>
              <BarChart data={metrics}>
                <CartesianGrid stroke="rgba(255,255,255,0.04)" />
                <XAxis dataKey="Ward_No" tick={{ fill: C.muted, fontSize: 9 }} tickLine={false} />
          ...
          <div style={{ overflowX: "auto", background: C.surface, borderRadius: 8, border: "1px solid rgba(0,212,255,0.08)" }}>
            <table style={{ width: "100%", borderCollapse: "collapse", fontSize: 12 }}>
              <thead>
                <tr style={{ borderBottom: "1px solid rgba(0,212,255,0.15)" }}>
                  {["Zone", "Name", "Supply R²", "RMSE (MLD)"].map(h => (
                    <th key={h} style={{ padding: "10px 12px", textAlign: "left", color: C.supply, fontSize: 10, letterSpacing: 1 }}>{h}</th>
                  ))}
                </tr>
              </thead>
...
function DataFilesPage() {
  ...
  const handleDownload = (id) => {
    fetch(`${API}/admin/files/${id}/download`, {
  ...
  return (
    <div>
      <h2 style={{ color: "white", marginBottom: 4 }}>📁 Water Data Files</h2>
      <p style={{ color: C.muted, marginBottom: 20, fontSize: 13 }}>
        {isAdmin
          ? "Manage uploaded Excel/CSV files — preview, download, or delete."
          : "Browse and download water data Excel files."}
      </p>
      <ErrBanner msg={err} />
      <OkBanner  msg={ok}  />

      {/* Built-in downloads */}
      <div style={{ background: C.surface, border: "1px solid rgba(0,212,255,0.12)", borderRadius: 10, padding: "14px 18px", marginBottom: 22 }}>
        <h4 style={{ color: C.supply, margin: "0 0 12px 0", fontSize: 13, letterSpacing: 1 }}>⬇ BUILT-IN DATASET DOWNLOADS</h4>
        <div style={{ display: "flex", gap: 10, flexWrap: "wrap" }}>
          <button
            onClick={() => dlBuiltin("/download/historical_excel", "WMA_Historical_City.xlsx")}
            style={{ background: "rgba(0,194,255,0.1)", border: "1px solid rgba(0,194,255,0.35)", color: C.supply, padding: "8px 14px", borderRadius: 7, fontSize: 12, fontWeight: 700, cursor: "pointer", fontFamily: "inherit" }}
          >
          ...
          {[1, 2, 3, 4, 5].map(z => (
            <button
              key={z}
              onClick={() => dlBuiltin(`/download/ward_excel/${z}`, `WMA_Zone${z}.xlsx`)}
              style={{ background: "rgba(0,255,136,0.08)", border: "1px solid rgba(0,255,136,0.25)", color: C.consumption, padding: "8px 12px", borderRadius: 7, fontSize: 12, fontWeight: 600, cursor: "pointer", fontFamily: "inherit" }}
            >
              Zone {z}
            </button>
          ))}
      ...
      {loading ? (
        ...
        <div style={{ textAlign: "center", padding: 50, color: C.muted, fontSize: 14 }}>
          {isAdmin ? "No files uploaded yet. Go to Upload Data to add files." : "No data files uploaded by admins yet."}
        </div>
      ) : (
                      ...
                      <thead>
                        <tr style={{ borderBottom: "1px solid rgba(0,212,255,0.15)" }}>
                          {preview.data.columns.map(col => (
                            <th key={col} style={{ padding: "7px 10px", textAlign: "left", color: C.supply, fontSize: 10, whiteSpace: "nowrap" }}>{col}</th>
                          ))}
                        </tr>
                      </thead>
                      <tbody>
...
function UploadPage() {
  ...
  return (
    <div>
      <h2 style={{ color: "white", marginBottom: 4 }}>⬆ Upload Water Data</h2>
      <p style={{ color: C.muted, marginBottom: 20, fontSize: 13 }}>
        Upload new Excel or CSV files containing ward-wise water data.
      </p>
      <ErrBanner msg={err} />
      <OkBanner  msg={ok}  />
      ...
      <div style={{ display: "grid", gridTemplateColumns: "repeat(auto-fit, minmax(300px, 1fr))", gap: 24 }}>
        {/* Upload form */}
        <div style={{ background: C.surface, border: "1px solid rgba(168,85,247,0.2)", borderRadius: 12, padding: 24 }}>
          <h4 style={{ color: C.admin, margin: "0 0 18px 0", fontSize: 13, letterSpacing: 1 }}>⚙ UPLOAD NEW FILE</h4>
          ...
          <form onSubmit={handleUpload}>
            <div
              ...
              style={{
                ...
                cursor: "pointer",
                marginBottom: 14,
                background: file ? C.adminDim : "transparent",
                transition: "all 0.2s",
              }}
            >
              ...
              {file
                ? <div style={{ color: C.muted, fontSize: 11, marginTop: 3 }}>{(file.size / 1024).toFixed(1)} KB</div>
                : <div style={{ color: C.muted, fontSize: 11, marginTop: 3 }}>Supports .xlsx, .xls, .csv</div>
              }
            </div>
            <input
              id="fileInput"
        ...
        {/* Requirements */}
        <div style={{ background: C.surface, border: "1px solid rgba(0,212,255,0.12)", borderRadius: 12, padding: 24 }}>
          <h4 style={{ color: C.supply, margin: "0 0 14px 0", fontSize: 13, letterSpacing: 1 }}>📋 FILE REQUIREMENTS</h4>
          {[
            ...
            <div key={k} style={{ borderBottom: "1px solid rgba(255,255,255,0.05)", padding: "8px 0", display: "flex", gap: 10 }}>
              <span style={{ color: C.muted, fontSize: 12, minWidth: 80 }}>{k}:</span>
              <span style={{ color: "white", fontSize: 12 }}>{v}</span>
            </div>
          ))}

          {result && (
            <div style={{ marginTop: 16, background: "rgba(0,255,136,0.06)", border: "1px solid rgba(0,255,136,0.2)", borderRadius: 8, padding: "12px 14px" }}>
              <div style={{ color: C.consumption, fontWeight: 700, marginBottom: 6, fontSize: 13 }}>✓ Last Upload</div>
...
function UsersPage() {
  ...
  return (
    <div>
      <h2 style={{ color: "white", marginBottom: 4 }}>👥 Registered Users</h2>
      <p style={{ color: C.muted, marginBottom: 20, fontSize: 13 }}>
        All users registered on Water Management Analytics.
      </p>
      <ErrBanner msg={err} />
      {loading ? (
        <Spinner />
        ...
        <>
          <div style={{ display: "flex", gap: 12, flexWrap: "wrap", marginBottom: 20 }}>
            ...
            <KpiCard label="Admins"         value={users.filter(u => u.role === "admin").length}   color={C.admin}       />
              ...
              <thead>
                <tr style={{ borderBottom: "1px solid rgba(0,212,255,0.15)" }}>
                  {["#", "Name", "Email", "Role", "Registered"].map(h => (
                    <th key={h} style={{ padding: "11px 14px", textAlign: "left", color: C.supply, fontSize: 10, letterSpacing: 1 }}>{h}</th>
                  ))}
                </tr>
              </thead>
              <tbody>
...
// ─────────────────────────────────────────────────────────
// MAIN APP INNER  (rendered when logged in)
// ─────────────────────────────────────────────────────────
function AppInner() {
  ...
  return (
    <div style={{ background: C.bg, minHeight: "100vh", fontFamily: "'Syne', sans-serif", color: "white" }}>
        ...
        {page === "city" && (
          <div style={{
            ...
            <span style={{ color: isAdmin ? C.admin : C.supply, fontSize: 13 }}>
              {isAdmin
                ? "⚙ Admin — Upload Excel files, view analytics, manage users."
                : "🎓 Student — View water analytics and download Excel data files."}
            </span>
            <RoleBadge role={user.role} />
          </div>
        )}
      ...
      <div style={{ textAlign: "center", padding: "20px 0 30px", color: C.muted, fontSize: 11, fontFamily: "monospace", letterSpacing: 1 }}>
...
// ─────────────────────────────────────────────────────────
// ROOT EXPORT — wraps all providers
// ─────────────────────────────────────────────────────────
