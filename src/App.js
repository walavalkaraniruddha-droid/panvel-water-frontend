...
function DataFilesPage() {
  ...
  const loadFiles = () => {
    ...
    axios.get(`${API}/admin/files`, { headers: { Authorization: `Bearer ${user.token}` } })
      .then(r => setFiles(r.data)).finally(() => setLoading(false));
  };
  ...
  const download = (id, name) => {
    ...
    a.href = `${API}/admin/files/${id}/download?token=${user.token}`;
    a.download = name; a.click();
  };

  const dlBuiltin = (url, name) => {
    const a = document.createElement("a");
    a.href = `${API}${url}?token=${user.token}`;
    a.download = name; a.click();
  };
...
// ─────────────────────────────────────────────────────────────
// INSIGHTS BOX  (reusable, shown in Analytics tab)
// ─────────────────────────────────────────────────────────────
function InsightsBox() {
  ...
  const LIMITATIONS = [
    { icon: "📡", title: "No Live IoT Sensor Data",             desc: "All values are generated from official PMC PDF aggregate figures — not from real-time pipe sensors or smart meters." },
  ...
  const col = (items, accent, accentBg, borderCol) => (
    <div style={{
      background: accentBg,
      ...
      <div style={{ display: "flex", alignItems: "center", gap: 8, marginBottom: 16 }}>
        <span style={{ fontSize: 16 }}>{accent === "#00ff88" ? "✅" : "⚠️"}</span>
        <span style={{ color: accent, fontWeight: 800, fontSize: 11, letterSpacing: 1 }}>
          {accent === "#00ff88" ? "WHAT THIS DASHBOARD SHOWS" : "KNOWN LIMITATIONS"}
        </span>
      </div>
      {items.map((f, i) => (
        <div key={i} style={{
          ...
          borderBottom: i < 4 ? `1px solid ${borderCol}` : "none",
        }}>
  ...
  return (
    <div style={{
      marginTop: 28, background: "#0d1a2b",
      border: "1px solid rgba(0,212,255,0.15)",
      ...
      <div style={{ marginBottom: 22 }}>
        <div style={{
          ...
          color: "#00c2ff", fontSize: 10, fontFamily: "monospace", letterSpacing: 3,
        ...
        <p style={{ color: C.muted, fontSize: 12, margin: "5px 0 0 0" }}>
          5 key features and 5 known limitations of the Water Management Analytics system
        </p>
      </div>

      <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 20 }}>
        {col(FEATURES,    "#00ff88", "rgba(0,255,136,0.04)",  "rgba(0,255,136,0.2)")}
        {col(LIMITATIONS, "#ff4d6d", "rgba(255,77,109,0.04)", "rgba(255,77,109,0.2)")}
      </div>
        ...
        <div>
          {cityData.length === 0 && wardData.length === 0 ? (
            <div style={{ textAlign: "center", color: C.muted, padding: "60px 20px" }}>
              ...
              <p style={{ fontSize: 13, marginBottom: 24 }}>Run a City or Ward prediction first to see live analytics here.</p>
              <div style={{ display: "flex", gap: 12, justifyContent: "center" }}>
                ...
                }}>🏙 Go to City Forecast</button>
                <button onClick={() => setTab("ward")} style={{
                  background: "rgba(0,255,136,0.1)", border: "1px solid rgba(0,255,136,0.3)",
                  color: C.consumption, padding: "10px 24px", borderRadius: 5,
                  fontWeight: 700, fontSize: 13, cursor: "pointer",
                }}>🏘 Go to Ward Forecast</button>
              </div>
            </div>
          ) : (
            <>
              ...
              {cityData.length > 0 && (
                <>
                  <div style={{ display: "flex", alignItems: "center", gap: 12, marginBottom: 18 }}>
                    <div style={{ width: 4, height: 24, background: C.supply, borderRadius: 2 }} />
                    <h3 style={{ margin: 0, color: "#ccd6f6", fontSize: "1rem" }}>
                      City Forecast Analytics — {cityData.length}-Day Prediction
                    </h3>
                    <span style={{
                      marginLeft: "auto", background: "rgba(0,194,255,0.1)",
                      border: "1px solid rgba(0,194,255,0.3)", color: C.supply,
                      fontSize: 10, fontFamily: "monospace", letterSpacing: 2,
                      padding: "3px 10px", borderRadius: 2,
                    }}>
                      {cityData[0]?.Date} → {cityData[cityData.length-1]?.Date}
                    </span>
                  </div>

                  {/* KPIs */}
                  <div style={{ display: "flex", gap: 12, flexWrap: "wrap", marginBottom: 24 }}>
                    <KpiCard label="Avg Supply"      value={avgOf(cityData, "Predicted_Supply_MLD")}      unit="MLD" color={C.supply} />
                    <KpiCard label="Avg Consumption" value={avgOf(cityData, "Predicted_Consumption_MLD")} unit="MLD" color={C.consumption} />
                    <KpiCard label="Avg Leakage"     value={avgOf(cityData, "Predicted_Leakage_MLD")}     unit="MLD" color={C.leakage} />
                    <KpiCard label="Avg Leakage %"   value={avgOf(cityData, "Leakage_Percentage")}         unit="%" color={C.warning} />
                    <KpiCard label="Forecast Days"   value={cityData.length}                              color={C.supply} sub="Days predicted" />
                  </div>

                  {/* Supply vs Consumption */}
                  <h4 style={{ color: "#ccd6f6", marginBottom: 10, fontSize: 13 }}>📈 Supply vs Consumption Forecast</h4>
                  <div style={{ background: C.surface, borderRadius: 10, padding: 20, marginBottom: 20, border: "1px solid rgba(0,212,255,0.1)" }}>
                    <ResponsiveContainer width="100%" height={260}>
                      <AreaChart data={sampleCity}>
                        ...
                        <Area type="monotone" dataKey="Predicted_Supply_MLD"      stroke={C.supply}      fill="url(#agS)" strokeWidth={2} name="Supply"      dot={false} />
                        <Area type="monotone" dataKey="Predicted_Consumption_MLD" stroke={C.consumption} fill="url(#agC)" strokeWidth={2} name="Consumption" dot={false} />
                      </AreaChart>
                    </ResponsiveContainer>
                  </div>

                  {/* Leakage bar (colour-coded by severity) */}
                  <h4 style={{ color: "#ccd6f6", marginBottom: 10, fontSize: 13 }}>🔴 Leakage Forecast — Severity Coloured</h4>
                  <div style={{ background: C.surface, borderRadius: 10, padding: 20, marginBottom: 20, border: "1px solid rgba(0,212,255,0.1)" }}>
                    <ResponsiveContainer width="100%" height={200}>
                      <BarChart data={sampleCity}>
                        <CartesianGrid stroke="rgba(255,255,255,0.04)" />
                  ...
                  {/* Leakage % trend */}
                  <h4 style={{ color: "#ccd6f6", marginBottom: 10, fontSize: 13 }}>⚠️ Leakage % Trend</h4>
                  <div style={{ background: C.surface, borderRadius: 10, padding: 20, marginBottom: 28, border: "1px solid rgba(0,212,255,0.1)" }}>
                    <ResponsiveContainer width="100%" height={180}>
                      <AreaChart data={sampleCity}>
                        ...
                        <CartesianGrid stroke="rgba(255,255,255,0.04)" />
                        <XAxis dataKey="Date" tick={{ fill: C.muted, fontSize: 10 }} tickLine={false} interval={Math.floor(sampleCity.length / 8)} />
                        <YAxis tick={{ fill: C.muted, fontSize: 10 }} tickLine={false} unit="%" width={50} />
                        <Tooltip content={<CustomTooltip />} />
              ...
              {wardData.length > 0 && (
                <>
                  <div style={{ display: "flex", alignItems: "center", gap: 12, marginBottom: 18 }}>
                    <div style={{ width: 4, height: 24, background: C.consumption, borderRadius: 2 }} />
                    <h3 style={{ margin: 0, color: "#ccd6f6", fontSize: "1rem" }}>
                      Zone Forecast Analytics — Ward {wardData[0]?.Ward_No} · {wardData[0]?.Ward_Name}
                    </h3>
                    <span style={{
                      marginLeft: "auto", background: "rgba(0,255,136,0.08)",
                      border: "1px solid rgba(0,255,136,0.25)", color: C.consumption,
                      fontSize: 10, fontFamily: "monospace", letterSpacing: 2,
                      padding: "3px 10px", borderRadius: 2,
                    }}>WARD · {wardData.length} DAYS</span>
                  </div>

                  <div style={{ display: "flex", gap: 12, flexWrap: "wrap", marginBottom: 24 }}>
                    <KpiCard label="Zone"           value={wardData[0]?.Ward_Name}                                  color={C.supply} />
                    <KpiCard label="Avg Supply"     value={avgOf(wardData, "Predicted_Supply_MLD")}     unit="MLD" color={C.supply} />
                    <KpiCard label="Avg Consumption" value={avgOf(wardData, "Predicted_Consumption_MLD")} unit="MLD" color={C.consumption} />
                    <KpiCard label="Avg Leakage"    value={avgOf(wardData, "Predicted_Leakage_MLD")}    unit="MLD" color={C.leakage} />
                    <KpiCard label="Leakage %"      value={wardData[0]?.Leakage_Percentage}             unit="%" color={C.warning} />
                  ...
                  <h4 style={{ color: "#ccd6f6", marginBottom: 10, fontSize: 13 }}>📈 Zone Supply · Consumption · Leakage</h4>
                  <div style={{ background: C.surface, borderRadius: 10, padding: 20, marginBottom: 20, border: "1px solid rgba(0,212,255,0.1)" }}>
                    <ResponsiveContainer width="100%" height={260}>
                      <AreaChart data={sampleWard}>
                        <defs>
                        ...
                        <Tooltip content={<CustomTooltip />} />
                        <Legend wrapperStyle={{ color: C.muted, fontSize: 11 }} />
                        <Area type="monotone" dataKey="Predicted_Supply_MLD"      stroke={C.supply}      fill="url(#awS)" strokeWidth={2} name="Supply"      dot={false} />
                        <Area type="monotone" dataKey="Predicted_Consumption_MLD" stroke={C.consumption} fill="none"       strokeWidth={2} name="Consumption" dot={false} />
                        <Area type="monotone" dataKey="Predicted_Leakage_MLD"     stroke={C.leakage}     fill="none"       strokeWidth={2} name="Leakage"     dot={false} />
                      </AreaChart>
                    </ResponsiveContainer>
                  ...
                  <h4 style={{ color: "#ccd6f6", marginBottom: 10, fontSize: 13 }}>🔴 Zone Leakage Forecast</h4>
                  <div style={{ background: C.surface, borderRadius: 10, padding: 20, marginBottom: 28, border: "1px solid rgba(0,212,255,0.1)" }}>
                    <ResponsiveContainer width="100%" height={180}>
                      <BarChart data={sampleWard}>
                        <CartesianGrid stroke="rgba(255,255,255,0.04)" />
                        <XAxis dataKey="Date" tick={{ fill: C.muted, fontSize: 10 }} tickLine={false} interval={Math.floor(sampleWard.length / 8)} />
                        <YAxis tick={{ fill: C.muted, fontSize: 10 }} tickLine={false} unit=" MLD" width={60} />
                        <Tooltip content={<CustomTooltip />} />
                        <Bar dataKey="Predicted_Leakage_MLD" name="Leakage" radius={[3, 3, 0, 0]} fill={C.leakage} />
                      </BarChart>
                    </ResponsiveContainer>
                  </div>
                </>
              ...
              {/* ── Insights Box (always shown after any prediction) ── */}
              <InsightsBox />
            </>
          )}
        </div>
