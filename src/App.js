...
function UploadPage() {
  ...
  const upload = async () => {
    ...
    try {
      // Use fetch (not axios) — avoids axios CORS preflight issues with multipart
      const res = await fetch(`${API}/admin/upload_excel`, {
        method: "POST",
        headers: { Authorization: `Bearer ${user.token}` },
      ...
      if (!res.ok) {
        setStatus({ ok: false, msg: `Upload failed (${res.status}): ${data.error || "Unknown error"}` });
        return;
      }
      setStatus({ ok: true, msg: `✅ Uploaded — ${data.rows} rows · ${data.columns?.join(", ")}` });
      setFile(null); setDesc("");
    } catch (e) {
      setStatus({ ok: false, msg: `Network error: ${e.message} — Check if backend is awake` });
    } finally { setLoading(false); }
  };
  ...
  return (
    <div style={{ padding: "28px 24px" }}>
      ...
      <div style={{ display: "flex", gap: 14, flexWrap: "wrap", marginBottom: 28 }}>
        ...
        <KpiCard label="Users"       value={students}     color={C.consumption} />
      </div>
